...
class TestModel(keras_parameterized.TestCase):
    ...
    def test_var_shape(self):
        model = VanTiny(weights=None, include_top=False, input_shape=(None, None, 3))
        model.compile(optimizer='rmsprop', loss='mse', run_eagerly=testing_utils.should_run_eagerly())

        images = np.random.random((10, 512, 384, 3)).astype('float32')
        labels = (np.random.random((10, 16, 12, 256)) + 0.5).astype('int32')
        model.fit(images, labels, epochs=1, batch_size=2)
